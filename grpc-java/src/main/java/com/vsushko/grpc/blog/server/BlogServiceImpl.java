...
public final class BlogServiceImpl extends BlogServiceGrpc.BlogServiceImplBase {
    ...
    public BlogServiceImpl(MongoClient client) {
        MongoDatabase mongoDatabase = client.getDatabase("blogdb");
    ...
    public void createBlog(Blog request, StreamObserver<BlogId> responseObserver) {
        ...
        Document document = new Document("author", request.getAuthor()).append("title", request.getTitle()).append("content", request.getContent());
    ...
    public void updateBlog(Blog request, StreamObserver<Empty> responseObserver) {
        ...
        String id = request.getId();
        Document result = mongoCollection.findOneAndUpdate(eq("_id", new ObjectId(id)), combine(set("author", request.getAuthor()), set("title", request.getTitle()), set("content", request.getContent())));
        if (result == null) {
            System.out.println("Blog not found");
            responseObserver.onError(error(Status.NOT_FOUND, BLOG_WAS_NOT_FOUND, "BlogId: " + id));
            return;
    ...
    private Blog documentToBlog(Document document) {
        return Blog.newBuilder().setAuthor(document.getString("author")).setTitle(document.getString("title")).setContent(document.getString("content")).setId(document.getObjectId("_id").toString()).build();
    }
}
